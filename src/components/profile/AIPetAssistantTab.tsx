...
import {
  ...
  Activity, Brain, PawPrint, RefreshCw, Lightbulb,
} from "lucide-react";
...
type AIMode = "assistant" | "tips" | "analysis" | "recipes" | "fun_facts";
...
export default function AIPetAssistantTab() {
  ...
  const handleModeChange = (newMode: AIMode) => {
    ...
    if (newMode !== "assistant" && newMode !== "analysis") {
      // Auto-fetch for non-chat modes if empty
  ...
  const fetchAnalysis = async () => {
    if (!user) return;
    // Load treatment logs to send context
